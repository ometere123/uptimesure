...
- `monitor-due`: claims due guarantees through an atomic lease, performs bounded HTTPS probes against validated targets, stores hashed evidence, and submits observations onchain when contract state requires it — failures and the first healthy check after a failure. Healthy steady state is recorded offchain and costs no gas.
